use crate::schema::{Attribute, FieldKind, ScalarType, Schema};
...
impl {delegate} {{
    ...
    fn map_row(row: &ruest_db::sqlx::postgres::PgRow) -> Result<{struct_name}, RuestDbError> {{
        Ok({struct_name} {{
{map_row}        }})
    }}
...
fn scalar_rust_type(field: &crate::schema::Field) -> String {
    match &field.kind {
...
fn generate_update_apply(model: &crate::schema::Model) -> String {
...
fn generate_update_set_sql(model: &crate::schema::Model) -> String {
...
fn generate_update_binds(model: &crate::schema::Model) -> String {
    let mut s = String::new();
    for field in table_columns(model) {
        if field.attributes.iter().any(|a| matches!(a, Attribute::Id)) {
            continue;
