...
impl std::fmt::Display for AppError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AppError::BadRequest(m) => write!(f, "[Ruest] Bad Request: {m}"),
            AppError::Unauthorized(m) => write!(f, "[Ruest] Unauthorized: {m}"),
            AppError::Forbidden(m) => write!(f, "[Ruest] Forbidden: {m}"),
            AppError::NotFound(m) => write!(f, "[Ruest] Not Found: {m}"),
            AppError::Conflict(m) => write!(f, "[Ruest] Conflict: {m}"),
            AppError::Internal(m) => write!(f, "[Ruest] Internal Error: {m}"),
        }
    }
}
