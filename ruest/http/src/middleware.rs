...
pub type RuestRequest = Request<RequestBody>;

/// Response type for middleware chain.
pub type RuestResponse = axum::response::Response;
...
pub trait Middleware: Send + Sync + 'static {
    fn handle(
        &self,
        request: RuestRequest,
        next: Next<axum::routing::Route>,
    ) -> Pin<Box<dyn Future<Output = RuestResponse> + Send>>;
}
