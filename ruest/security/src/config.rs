...
impl SecurityConfig {
    ...
    pub fn dev() -> Self {
        Self {
            jwt_secret: "ruest-dev-secret-change-in-production!!".into(),
            jwt_expires_in_secs: 3600,
            jwt_issuer: Some("ruest".into()),
            public_routes: vec![
                "/health".into(),
                "/auth/login".into(),
                "/auth/register".into(),
    ...
    pub fn from_env() -> Result<Self, SecurityError> {
        let jwt_secret = env::var("RUEST_JWT_SECRET").map_err(|_| {
            SecurityError::Config(
                "RUEST_JWT_SECRET is not set (use SecurityConfig::dev() for local dev)".into(),
            )
        })?;

        let jwt_expires_in_secs = env::var("RUEST_JWT_EXPIRES_IN_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(3600);

        let jwt_issuer = env::var("RUEST_JWT_ISSUER").ok();
        ...
        if let Ok(extra) = env::var("RUEST_PUBLIC_ROUTES") {
