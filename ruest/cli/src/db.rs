...
pub enum DbCommands {
    /// Initialise `schema.ruest` et `ruestdb/migrations/`
    Init,
...
/// `ruest generate` — alias de `ruest db generate`
pub fn cmd_generate() -> Result<()> {
    generate_client(&project_root()).map_err(map_err)
}

pub fn doctor_db_hints() {
    println!("  ruest db init           — schema.ruest + migrations/");
    println!("  ruest generate          — client Rust type-safe");
    println!("  ruest migrate dev       — migration + apply");
