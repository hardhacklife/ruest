...
pub fn bootstrap<M: Module>(root: M) -> Result<RuestApplication, CoreError> {
    let container = Container::new();
    let mut ctx = ModuleContext::new(container.clone());
    root.configure(&mut ctx)?;
    Ok(RuestApplication::new(ctx.container))
}
