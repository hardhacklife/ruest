...
use rustforge::http::axum::body::Body;
use rustforge::http::axum::http::{Request, StatusCode};
use rustforge::http::axum::Router;
use rustforge::core::bootstrap;
use rustforge::prelude::*;
use rustforge::testing::TestFactory;
...
async fn http_get_ping_returns_json() {
    ...
    let body = rustforge::http::axum::body::to_bytes(response.into_body(), usize::MAX)
        .await
        .unwrap();
...
#[test]
fn forge_err_builds_app_error() {
    let err = forge_err!(NotFound, "absent");
