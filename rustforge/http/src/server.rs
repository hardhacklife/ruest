...
use axum::Router;
use rustforge_core::RustForgeApplication;
...
pub async fn serve(app: RustForgeApplication, router: Router) -> Result<(), HttpError> {
    ...
    tracing::info!("RustForge listening on http://{addr}");
...
fn finalize_router(app: RustForgeApplication, router: Router) -> Result<BuiltRouter, HttpError> {
