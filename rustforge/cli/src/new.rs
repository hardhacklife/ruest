...
pub fn cmd_new(name: &str, template: Option<&str>, rustforge_path: &str) -> Result<()> {
    ...
    fs::write(dir.join("Cargo.toml"), templates::app_cargo(name, rustforge_path))?;
...
// Ajoutez un gateway WebSocket : forge g resource chat
"#,
            )?;
        }
    }
    ...
    println!("✓ Created Forge app `{name}` (template: {template_name})");
    println!("  cd {name}");
    println!("  forge start");
    Ok(())
}

pub fn detect_rustforge_path() -> String {
    if Path::new("rustforge").exists() {
        "../rustforge".into()
    } else {
        "../rustforge".into()
    }
