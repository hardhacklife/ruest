...
impl JwtService {
    ...
    pub fn sign(&self, claims: &ForgeClaims) -> Result<String, SecurityError> {
        encode(&Header::default(), claims, &self.encoding).map_err(SecurityError::from)
    }

    pub fn verify(&self, token: &str) -> Result<ForgeClaims, SecurityError> {
        let data = decode::<ForgeClaims>(token, &self.decoding, &self.validation)
