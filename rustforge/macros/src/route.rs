...
pub fn expand_routes_impl(item: TokenStream) -> TokenStream {
    ...
    let route_mounts: Vec<_> = route_methods.iter().map(|route| {
        ...
        quote! {
            {
                let path = ::rustforge::router::join_paths(Self::PREFIX, #suffix);
                router = router.route(
                path,
                ::rustforge::http::axum::routing::#route_fn({
                    let controller = std::sync::Arc::clone(&controller);
                    move || {
                        let controller = std::sync::Arc::clone(&controller);
                        async move {
                            ::rustforge::http::axum::response::IntoResponse::into_response(
